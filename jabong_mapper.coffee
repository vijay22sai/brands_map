...
mongoUrl = "mongodb://#{host}:27017/jabongDB"
...
appStream.on "data", (doc) ->
  ...
  for brand in brands
    ...
    client.sadd("jabong_brands", brand, (err) -> console.log(err) if err?)
    if category?
      client.sadd("categories", category, (err) -> console.log(err) if err?)
      client.sadd("jabong_categories", category, (err) -> console.log(err) if err?)
